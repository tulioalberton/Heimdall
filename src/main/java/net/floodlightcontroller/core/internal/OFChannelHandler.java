...
class OFChannelHandler extends SimpleChannelInboundHandler<Iterable<OFMessage>> {
	...
	public abstract class OFChannelState {
		...
		void processOFMessage(OFMessage m)
			...
			if (!state.channelHandshakeComplete) {
				...
				log.trace("Received {} message at inconplete handshake.", m.getType());
				...
				case ECHO_REPLY:
					...
					break;
				case ECHO_REQUEST:
					//log.debug("Received an {} message.", m.getType());
				...
				switch(m.getType()){
				case ECHO_REPLY:
					log.trace("Received: ECHO_REPLY message: {}", m);
					processOFEchoReply((OFEchoReply)m);
					break;
				case ECHO_REQUEST:
					log.trace("Received: ECHO_REQUEST message: {}", m);
					processOFEchoRequest((OFEchoRequest)m);
					break;
					// Send to SwitchManager and thus higher orders of control
				default:
					log.trace("Received: {} message.", m.getType());
	...
	private void sendHelloMessage() throws IOException {
		...
		OFHello m = builder.setXid(handshakeTransactionIds--)
				.build();
		log.debug("Sending: {} message.", m.getType());
		write(m);
	...
	private void sendEchoRequest() {
		...
		//log.debug("Sending Echo Request, echoSendTime:{}", echoSendTime);
		write(request);
	}
	...
	private void sendEchoReply(OFEchoRequest request) {
		OFEchoReply reply = factory.buildEchoReply()
				...
				.build();
		//log.debug("Received {} message, Sending {} message.", request.getType(), reply.getType());
		write(reply);		
	}

	private void write(OFMessage m) {
		//log.trace("Write and Flushing message into channel, Message:{}",m);
