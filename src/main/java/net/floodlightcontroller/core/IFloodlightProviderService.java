...
public interface IFloodlightProviderService extends
   ...
   /**
    * Tulio Ribeiro
    * Add a rollback listener to the controller
    * 
    * Each message re-initiate in controller 
    * "handleMessage(IOFSwitch sw, OFMessage m, FloodlightContext bContext)"
    * will be informed to registered modules.  
    * 
    * @param listener
    */
   void addRollbackListener(IControllerRollbackListener listener);

   /**
    * Initial recover of each data Structure registered as a persistent module 
    * @param dataStructure, name of data structure used at CreationTime. 
    * @param m, used mapTracing.
    */
   public <K, V> void doInitialRecover(String dataStructure, MapTracing<K, V> m);
   
   /**
    * When we have a threshold of rollback we can inform about contention and try a global lock 
    * or ask to controller wait a little.
    */
   public void informContention(Update update);
   
   
   /**
    * Remove a rollback listener from the controller
    * 
    * @param listener
    */
   void removeRollbackListener(IControllerRollbackListener listener);
