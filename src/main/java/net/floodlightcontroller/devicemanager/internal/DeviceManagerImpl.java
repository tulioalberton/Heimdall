...
import net.floodlightcontroller.heimdall.tracing.json.JsonConvertions;
...
public class DeviceManagerImpl implements IDeviceService, IOFMessageListener, ITopologyListener, IFloodlightModule, IEntityClassListener, IInfoProvider {
	...
	//private MapTracing<String, Long> dataWrite;//temporary
	...
	public void init(FloodlightModuleContext fmc) throws FloodlightModuleException {
		...
		//this.tarService = fmc.getServiceImpl(ITarService.class);
	...
	public void startUp(FloodlightModuleContext fmc)
		...
		/*dataWrite = tarService.createMapTracing(new HashMap<String, Long>(), "WriteTaxa:DM", 
				net.floodlightcontroller.heimdall.ITarService.Scope.GLOBAL, null,
				serializer_K, deserializer_K, serializer_V, deserializer_V);*/
		
		
		
		
		Function<Device, String> serializer = (dev) -> {
			try {
				return new ObjectMapper().writeValueAsString(dev);
			} catch (JsonProcessingException e) {
				throw new RuntimeException(e);
			}
		};
		
		//BiFunction<Long, String, Device> deserializer = JsonConvertions::deserDevice;
		BiFunction<Long, String, Device> deserializer = (key, json) -> {
			return JsonConvertions.deserDevice(this, entityClassifier, key, json);
		};
		
		// primitivos
		// ser: String::valueOf
		// deser: (_key, str) -> Long.parseLong(str)
		
		deviceMap = new ConcurrentHashMap<Long, Device>();
		/*deviceMap = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<Long, Device>(),
				"DM:deviceMap",
				net.floodlightcontroller.heimdall.ITarService.Scope.LOCAL, 
				null,
				serializer,
				deserializer);*/
	...
	protected Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		if (eth.getPayload() instanceof BSN || eth.getPayload() instanceof LLDP) {
			logger.info("\nCONTINUING PIPELINE, BSN or LLDP\n");
			return Command.CONTINUE;
		} 
		
		if (srcEntity == null) {
			//logger.info("\n\nStopping PIPELINE, srcEntity == null\n");
		...
		if (eth.getDestinationMACAddress().getLong() == 0) {
			...
			logger.info("Stopping PIPELINE, srcEntity == null");
			return Command.STOP;
		}
			...
			if (deviceKey != null) {
				...
				if (device == null) {
					...
					//continue;
						...
						logger.info("Stopping while(true) inside DeviceManagerImpl.java."); 
						break;						
					}
				...
				/*if (dataWrite.get(deviceKey.toString()) != null) {
					//logger.info("Counting for DM: Device: {}", device.toString());
					dataWrite.put(deviceKey.toString(), dataWrite.get(deviceKey.toString()) + 1); 
				} else {
					dataWrite.put(deviceKey.toString(), 1L);
				}*/
