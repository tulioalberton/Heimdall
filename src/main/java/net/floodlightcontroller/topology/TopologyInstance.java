    ...
    private Archipelago getArchipelago(DatapathId d) {
        for (Archipelago a : archipelagos) {
        	try {
				...
				log.error("NPE at getArchipelago ({}) ",d);
			}
        ...
        for (DatapathId sw : this.switches) {
            for (OFPort p : this.portsPerSwitch.get(sw)) { /* includes edge and link ports */
                ...
                if (isEdge(sw, p)) {
                    ...
                    try{
                    	log.debug("Trying to get archipelago for switch:{} ", sw.getLong());
                    	aId = getArchipelago(sw).getId();                    	
                    }
                    catch(NullPointerException npe){
                    	log.error("NUllPointerException at getArchipelago({}).getId(), This should not happen.. :/", sw.getLong());
                    	compute();
                    }                    
