...
ITopologyManagerBackend, ILinkDiscoveryListener, IOFMessageListener {
    ...
    protected int TOPOLOGY_COMPUTE_INTERVAL_MS = 70;
  ...
  /*  public int getTopologyComputeInterval() {
   ...
   /* public void setTopologyComputeInterval(int time_ms) {
    ...
    public synchronized boolean updateTopology(String reason, boolean forced) {
        ...
        log.debug("Recomputing topology due to: {}", reason);
    ...
    public void linkDiscoveryUpdate(List<LDUpdate> updateList) {
        ...
        updateTopology("link-discovery-update-event", true);
    }
    ...
    public Command receive(IOFSwitch sw, OFMessage msg, FloodlightContext cntx) {
        ...
        case PACKET_IN:
            ...
            //log.info("Received packetIn... TOPOLOGYMANAGER..");
    ...
    public void startUp(FloodlightModuleContext context) {
		...
		summaryTopology.reschedule(10000, TimeUnit.MILLISECONDS);*/
	}
	...
	protected class showDebugInfo implements Runnable {
		@Override
		public void run() {
			log.info("{}",getAllDebugInfo());			
			summaryTopology.reschedule(20000, TimeUnit.MILLISECONDS);
		}
	}
    ...
    public void doMultiActionPacketOut(byte[] packetData, IOFSwitch sw,
        ...
        actions.add(sw.getOFFactory().actions().output(OFPort.IN_PORT, 0));
        ...
        log.info("write broadcast packet on switch-id={}, interaces={}", 
        		new Object[]{sw.getId(), ports});
        sw.write(pob.build(), LogicalOFMessageCategory.MAIN);
    }
    
    public void doMultiActionPacketOut2(OFPacketIn pi, IOFSwitch sw, Set<OFPort> ports, FloodlightContext cntx) {

        if (ports == null) return;

        log.info("write broadcast MODEL 2 packet on switch-id={}, interaces={}", new Object[]{sw.getId(), ports});
        sw.write(pi, LogicalOFMessageCategory.MAIN);
        //sw.write(pob.build(), LogicalOFMessageCategory.MAIN);
    }
    ...
    protected void doFloodBDDP(DatapathId pinSwitch, OFPacketIn pi, FloodlightContext cntx) {
        ...
        for (DatapathId sid : switches) {
           ...
           */
            ...
            doMultiActionPacketOut2(pi, sw, ports, cntx);
        }
    ...
    protected Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
        ...
        if (eth.getPayload() instanceof BSN) {
            ...
            if (bsn.getPayload() instanceof LLDP == false){
            	log.info("bsn.getPayload() instanceof LLDP == false. Returing Command.CONTINUE");
                return Command.CONTINUE;
            }
        	...
        	log.info("Floodding LLDP packet. EtherType: {}", eth.getEtherType());
        	doFloodBDDP(sw.getId(), pi, cntx);
        	return Command.STOP;
        }else {
        	//log.info("Dropping filter on switch: {}, EtherType: {}", sw, eth.getEtherType());
	...
	public StringBuffer getAllDebugInfo() {
		...
		sb.append("\nTOPOLOGY MANAGER DEBUG:");
		
		sb.append("\nSWITCH PORTS:");
		Set<DatapathId> dpId = switchPorts.keySet();
		Iterator<DatapathId> itDpid = dpId.iterator();
		while(itDpid.hasNext()){
			DatapathId dpid =itDpid.next();
			sb.append("\n\tSwitch: "+dpid);
			Set<OFPort> of = switchPorts.get(dpid);
			Iterator<OFPort> itOf = of.iterator();
			while(itOf.hasNext())
				sb.append("\n\t\tPort: "+itOf.next());
		}
		...
		sb.append("\nSWITCH PORT LINKS:");
		Set<NodePortTuple> nptS = switchPortLinks.keySet();
		Iterator<NodePortTuple> itNPT = nptS.iterator();
		while(itNPT.hasNext()){
			NodePortTuple npt = itNPT.next();
			sb.append("\n\tNodePortTuple: "+npt);
			Set<Link> l = switchPortLinks.get(npt);
			Iterator<Link> itL = l.iterator();
			while(itL.hasNext())
				sb.append("\n\t\tLink: "+itL.next());
		}
		
		sb.append("\nINTER CLUSTER LINKS");
		Set<NodePortTuple> nptB = interClusterLinks.keySet();
		Iterator<NodePortTuple> itB = nptB.iterator();
		while(itB.hasNext()){
			NodePortTuple np = itB.next();
			sb.append("\n\tInter Cluster : "+np);
			Set<Link> lS = interClusterLinks.get(np); 
			Iterator<Link> itL = lS.iterator();
			while(itL.hasNext())
				sb.append("\n\t\tLink: "+itL.next());
		}
		
		sb.append("\nDIRECT LINKS:");
		Set<NodePortTuple> nptD = directLinks.keySet();
		Iterator<NodePortTuple> itD = nptD.iterator();
		while(itD.hasNext()){
			NodePortTuple nD = itD.next();
			sb.append("\n\t"+nD);
			Set<Link> lS = directLinks.get(nD);
			Iterator<Link> itLS = lS.iterator();
			while(itLS.hasNext())
				sb.append("\n\t\t"+itLS.next());
		}
		
		//sb.append("\n GET ALL LINKS: "+getAllLinks());
		return sb;
	}
