...
public class OFMessageDamper {
	 ...
	 * @param sw
	 * @param msg
	 * @return true if the message was written to the switch, false if the
	 *         message was dampened.
	 */
	public boolean write(IOFSwitch sw, OFMessage msg) {
		if (!msgTypesToCache.contains(msg.getType())) {
			...
			/**
			 * Tulio Ribeiro, LaSIGE, FCUL.
			 */
			if (msg.getType().equals(OFType.PACKET_OUT) || msg.getType().equals(OFType.FLOW_MOD)) {
				//log.info("Buffering 1 message. Type:{}, ThreadId:{}", msg.getType(), Thread.currentThread().getId());
				sw.write(msg, true);
			} else {
				//log.info("NOT Buffering message. Type:{}", msg.getType());
				sw.write(msg);
			}
			return true;
		}
        ...
        if (cache.update(entry)) {
            ...
            //log.info("Dampening cached msg, message. Type:{}, ThreadId:{}", msg.getType(), Thread.currentThread().getId());
            return false; 
		} else {
			log.debug("Not dampening new msg {}", msg);
			/**
			 * Tulio Ribeiro, LaSIGE. Storing packetOut and flowMod messages. We
			 * dispatch after pipeline packetIn chain
			 */
			if (msg.getType().equals(OFType.PACKET_OUT) || msg.getType().equals(OFType.FLOW_MOD)) {
				//log.info("Buffering2 message. Type:{}, ThreadId:{}", msg.getType(), Thread.currentThread().getId());
				sw.write(msg, true);
			} else {
				//log.info("NOT Buffering message. Type:{}", msg.getType());
				sw.write(msg);
