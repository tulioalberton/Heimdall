...
public class TimedCache<K> {    
	...
	private final ConcurrentMap<K, Long> cache;
	...
	public TimedCache(int capacity, int timeToLive) {
        cache = new ConcurrentLinkedHashMap.Builder<K, Long>().maximumWeightedCapacity(capacity)
            .build();
        this.timeoutInterval = timeToLive;
    }
	public TimedCache() {
        cache = new ConcurrentLinkedHashMap.Builder<K, Long>().maximumWeightedCapacity(64)
            .build();
        this.timeoutInterval = 3600000;//one hour
    }
    ...
    public boolean update(K key)
    {
        ...
        if (curr - prev > this.timeoutInterval) {
            if (cache.replace(key, prev, curr)) {
            	System.out.println("Cache invalid... threadId:"+Thread.currentThread().getId()+ "\n prev:"+prev+" curr:"+curr+ " Timeout:"+this.timeoutInterval);
            		return false;
            }
        }
