...
implements IFloodlightModule, IVirtualNetworkService, IOFMessageListener {
	...
	// Our internal state
	protected ConcurrentHashMapTracing<String, VirtualNetwork> vNetsByGuid; // List of all created virtual networks
	protected ConcurrentHashMapTracing<String, String> nameToGuid; // Logical name -> Network ID
	protected ConcurrentHashMapTracing<String, IPv4Address> guidToGateway; // Network ID -> Gateway IP
	protected ConcurrentHashMapTracing<IPv4Address, Set<String>> gatewayToGuid; // Gateway IP -> Network ID
	protected ConcurrentHashMapTracing<MacAddress, IPv4Address> macToGateway; // Gateway MAC -> Gateway IP
	protected ConcurrentHashMapTracing<MacAddress, String> macToGuid; // Host MAC -> Network ID
	protected ConcurrentHashMapTracing<String, MacAddress> portToMac; // Host MAC -> logical port name
	...
	public void init(FloodlightModuleContext context)
		...
		*/
		Function<VirtualNetwork, String> serVirtualNetwork = (info) -> {
			try {
		...
		Function<String, VirtualNetwork> desVirtualNetwork = (String str) -> {
			try {
				return new ObjectMapper().readValue(str, VirtualNetwork.class);
			} catch (IOException e) {
				throw new UncheckedIOException(e);
			}
		};		
		
		vNetsByGuid = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<String, VirtualNetwork>(), 
				"VN:vNetsByGuid", 
				Scope.GLOBAL, 
				this,
				serVirtualNetwork,
		...
		Function<IPv4Address, String> serIPv4 = (info) -> {
			try {
				return new ObjectMapper().writeValueAsString(info);
			} catch (JsonProcessingException e) {
				throw new RuntimeException(e);
		...
		Function<String, IPv4Address> desIPv4 = (String str) -> {
			try {
				return new ObjectMapper().readValue(str, IPv4Address.class);
			} catch (IOException e) {
				throw new UncheckedIOException(e);
			}
		};	
		
		guidToGateway = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<String, IPv4Address>(), 
				"VN:guidToGateway", 
				Scope.GLOBAL, this, serIPv4, desIPv4);
		macToGateway = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<MacAddress, IPv4Address>(), 
				"VN:macToGateway", 
				Scope.GLOBAL, this, serIPv4, desIPv4);
		
		
		Function<Set<String>, String> serSetString = (info) -> {
			try {
				return new ObjectMapper().writeValueAsString(info);
			} catch (JsonProcessingException e) {
				throw new RuntimeException(e);
		...
		Function<String, Set<String>> desSetString = (String str) -> {
			try {
				return new ObjectMapper().readValue(str, Set.class);
			} catch (IOException e) {
				throw new UncheckedIOException(e);
			}
		};
		
		gatewayToGuid = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<IPv4Address, Set<String>>(), 
				"VN:gatewayToGuid", Scope.GLOBAL, this, serSetString, desSetString);
		
		
		
		
		Function<String, String> desString = (String str) -> {
			try {
				return new ObjectMapper().readValue(str, String.class);
			} catch (IOException e) {
				throw new UncheckedIOException(e);
			}
		};
		
		nameToGuid = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<String, String>(), 
				"VN:nameToGuid", 
				Scope.GLOBAL, this, String::valueOf, desString);
		
		
		Function<MacAddress, String> serMacAddr = (info) -> {
			try {
				return new ObjectMapper().writeValueAsString(info);
			} catch (JsonProcessingException e) {
				throw new RuntimeException(e);
		...
		Function<String, MacAddress> desMacAddr = (String str) -> {
			try {
				return new ObjectMapper().readValue(str, MacAddress.class);
			} catch (IOException e) {
				throw new UncheckedIOException(e);
			}
		};
		
		macToGuid = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<MacAddress, String>(), 
				"VN:macToGuid", 
				Scope.GLOBAL, this, serMacAddr , desMacAddr);		
		portToMac = tarService.createConcurrentHashMapTracing(
				new ConcurrentHashMap<String, MacAddress>(), 
				"VN:portToMac", 
				Scope.GLOBAL, this, desMacAddr, serMacAddr);
		...
		public void deviceAdded(IDevice device) {
			...
			for (IPv4Address i : device.getIPv4Addresses()) {
				if (gatewayToGuid.containsKey(i)) {
					...
					if (log.isDebugEnabled())
						log.debug("Adding MAC {} with IP {} as a gateway",
								mac.toString(),
								i.toString());
